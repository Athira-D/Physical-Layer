...
//check for blank spaces in string data type
...
void parse(char *filename)
{
	...
	//printf("+++++++++++%d\n+++++++%d\n",first_blk,curr_blk);
	relcat(newfilename,count,no_records,first_blk,prev_blk,no_of_slots);
	fclose(file);
	fclose(disk);
}
...
// ----Modify the attribute catalog------
	...
	curr_blk=5;int n_attr;prev_blk=5;next_blk=-1;
	while(curr_blk!=-1)
	{
		n_attr=0;
		fseek(disk,curr_blk*2048+12,SEEK_SET);
