...
ddlCommand : CREATE TABLE ID '(' ColList ')' ';' 
					 { 
					fe_createrel($3, $5); freeall($3); freeall($5); 
					 }
	      | DROP TABLE ID ';'                  
					 {       cout<<"1";
						fe_droprel($3); freeall($3); 
					 }
	      | ALTER TABLE ID RENAME TO ID ';' 
					{ 
						fe_renamerel($3, $6); freeall($3), freeall($6); 
					}
	      | ALTER TABLE ID RENAME COLUMN ID TO ID ';' 
					{ 
					fe_renamecol($3, $6, $8); freeall($3); freeall($6); freeall($8); 
					}
	      | CREATE INDEX  ID '.' ID ';' 
					{
					 fe_createind($4, $6); freeall($4); freeall($6); 
					}
	      | DROP INDEX  ID '.' ID ';' {
					fe_dropind($4, $6); freeall($4); freeall($6);
					 }
			;
